...
/*
mremap系统调用被应用程序用来改变映射区段
(VMAs)的边界地址。一般的VMA覆盖至少一个内
存页(在i386架构上为4kB)，do_mremap()内核代码执
行重映射虚拟内存区域时发现缺少正确的边
界检查，可导致建立0字节长度的虚拟内存区
域。
 ...
 * 使对于malloc(0)，dlmalloc也会返回一块MINSIZE的内存给用户。
 */
...
/* 
 ...
 如果P-bit等于0则表示前一个邻接chunk是空闲的，
 可以通过previous chunk size来快速定位前一个相邻
 chunk的首地址。如果P-bit等于1，则不可以通过
 previous chunk size定位前一个相邻chunk的首地址，因
 为只有前一个chunk空闲时才会设置previous chunk size
 内容。
*/
...
/* 将以内存地址ptr +/- offset开始空间转换为mchunkptr。*/
 ...
 * 文件被映射到多个页上，如果文件的大小不
 * 是所有页的大小之和，最后一个页不被使用的
 * 空间将会清零。
 * 如果是由mmap分配的，则可以不清零，否则必须清零。
 * 
 * */
...
/* IS_MMAPPED_BIT 用于标记该SEGMENT 由mmap申请*/
#define is_mmapped_segment(S)  ((S)->sflags & IS_MMAPPED_BIT)
/* EXTERN_BIT 用于标记该SEGMENT 尚未分配*/
...
/*
The code
takes advantage of the fact that statically-allocated storage is zero-initialised by the operating
system when executables are loaded.
自动初始化为0
*/
...
/* ---------------------------- Indexing Bins ---------------------------- */
/* 判断s是否小于256。*/
/* 应该是判断s是否小于32吧 add by alex */
#define is_small(s)         (((s) >> SMALLBIN_SHIFT) < NSMALLBINS)
/* s >> 3，将s缩小8倍，根据大小求对应索引 */
#define small_index(s)      ((s)  >> SMALLBIN_SHIFT)

/* i << 3，将i扩大8倍，根据索引计算对应大小 */
#define small_index2size(i) ((i)  << SMALLBIN_SHIFT)

/* 求MIN_CHUNK_SIZE 对应的索引值*/
#define MIN_SMALL_INDEX     (small_index(MIN_CHUNK_SIZE))

/* addressing by index. See above about smallbin repositioning */
/* 
 * 为什么需要<<1 ??? 
 *  其直接将指针空间当sbinptr空间使用
 */

/* 
 * add by alex:
 * [(i) << 1] 为了越过prev_foot head 
 *
 */
...
/* 获取值为1的最小bit位
x中值为1的最小bit位为1，其它位都为0的掩码*/
#define least_bit(x)         ((x) & -(x))

/* mask with all bits to left of least bit of x on */
/* x中值为1的最小bit位的左边(不包括值为1的该
最小bit位) 所有位都为1，其它位都为0的掩码 */
#define left_bits(x)         ((x<<1) | -(x<<1))

/* mask with all bits to left of or equal to least bit of x on */
/* x中值为1的最小bit位的左边(包括值为1的该
最小bit位) 所有位都为1，其它位都为0的掩码 */
...
/* 获取mstate，见前一个mark_inuse_foot 宏*/
...
/* 例子: insert_chunk(fm, p, psize);
将chunk块P插入GM中*/
#define insert_chunk(M, P, S)\
  if (is_small(S)) insert_small_chunk(M, P, S)\
  else { tchunkptr TP = (tchunkptr)(P); insert_large_chunk(M, TP, S); }

/* 例子: unlink_chunk(fm, p, prevsize);
将chunk块P从GM中拆出
*/
...
static void* mmap_alloc(mstate m, size_t nb) {
  ...
  if (mmsize > nb) {     /* Check for wrap around 0 */
    ...
    if (mm != CMFAIL) {
      ...
      mchunkptr p = (mchunkptr)(mm + offset);		/* 向前偏移*/
      p->prev_foot = offset | IS_MMAPPED_BIT;		/* 伪装的 previous chunk*/
...
static void* prepend_alloc(mstate m, char* newbase, char* oldbase,
  ...
  if (oldfirst == m->top) {
    ...
    insert_chunk(m, q, qsize);		/* 再insert进来*/
  ...
  if (tbase != CMFAIL) {
    ...
    if (!is_initialized(m)) { /* first-time initialization */
      ...
      if (sp != 0 &&
        ...
        if (sp != 0 &&
            !is_extern_segment(sp) &&
            (sp->sflags & IS_MMAPPED_BIT) == mmap_flag) {	/* 合并*/
...
/* MAX_SMALL_REQUEST对齐值(大于256) <= nb < MAX_REQUEST对齐值  */
static void* tmalloc_large(mstate m, size_t nb) {
  tchunkptr v = 0;
  size_t rsize = -nb; /* Unsigned negation */	/* 初始，便于下面的比较*/
  tchunkptr t;
  bindex_t idx;
  compute_tree_index(nb, idx); /* 找到nb对应的最合适分配对应索引*/

  if ((t = *treebin_at(m, idx)) != 0) {	/* 获取树根*/
  	/* 遍历树查取size==nb的节点*/
    ...
    for (;;) {
      ...
      if (rt != 0 && rt != t)
        rst = rt;
      if (t == 0) {	/* 保存最近的一个大于nb的节点(即最接近nb大小)*/
  ...
  if (t == 0 && v == 0) { /* set t to root of next non-empty treebin */
  	/* 对应nb的索引treebin没有合适分配的，则在大于nb的treebin中去找*/
    binmap_t leftbits = left_bits(idx2bit(idx)) & m->treemap;
    if (leftbits != 0) {
      bindex_t i;
      binmap_t leastbit = least_bit(leftbits);	/* 最接近nb的*/
      compute_bit2idx(leastbit, i);	/* 最接近nb的bin 索引*/
      t = *treebin_at(m, i);			/* treebin 树节点*/
    }
  }

  while (t != 0) { /* find smallest of tree or subtree */
    size_t trem = chunksize(t) - nb;
    if (trem < rsize) {	/* 查找最合适的*/
      rsize = trem;
      v = t;
    }
    t = leftmost_child(t);
  ...
  /* 如果dv更适合分配则该函数会返回0，然后在dlmalloc函数内
  会直接在dv内进行分配内存*/
  if (v != 0 && rsize < (size_t)(m->dvsize - nb)) {
  	/* treebin内更适合分配内存，因此开始分配工作*/
...
static void* tmalloc_small(mstate m, size_t nb) {
  ...
  v = t = *treebin_at(m, i);
  /* nb < 256，而tree里的每个chunk块都大于256，所以
  此处不会参数负数，以下其它地方类似。*/
  rsize = chunksize(t) - nb;	
	...
	/* 树的任意节点I 空间大小存在如下规则:
	  ...
	  Using these rules, each node's left subtree contains all smaller
  	sizes than its right subtree.  However, the node at the root of each
  	subtree has no particular ordering relationship to either.
  ...
  if (RTCHECK(ok_address(m, v))) {
    ...
    assert(chunksize(v) == rsize + nb);
    if (RTCHECK(ok_next(v, r))) {
      unlink_large_chunk(m, v);		/* 移除v 。*/
      if (rsize < MIN_CHUNK_SIZE)	/* 剩余空间太小则直接全部分配*/
        set_inuse_and_pinuse(m, v, (rsize + nb));
      else {	/* 否则将剩余空间作为新的dv */
        set_size_and_pinuse_of_inuse_chunk(m, v, nb);
        set_size_and_pinuse_of_free_chunk(r, rsize);
...
static void* internal_realloc(mstate m, void* oldmem, size_t bytes) {
  if (bytes >= MAX_REQUEST) {		/* 分配失败*/
    MALLOC_FAILURE_ACTION;
    return 0;
  }
  if (!PREACTION(m)) {
    mchunkptr oldp = mem2chunk(oldmem);		/* 旧chunk块*/
    size_t oldsize = chunksize(oldp);				/* 旧块大小*/
    mchunkptr next = chunk_plus_offset(oldp, oldsize);		/* 下一块 */
    mchunkptr newp = 0;
    void* extra = 0;
    ...
    if (RTCHECK(ok_address(m, oldp) && ok_cinuse(oldp) &&
      ...
      if (is_mmapped(oldp))
        newp = mmap_resize(m, oldp, nb);				/* 进行resize mmap*/
      else if (oldsize >= nb) { /* already big enough */	/* 缩减，直接末尾裁剪*/
        size_t rsize = oldsize - nb;		/* 剩余大小*/
        newp = oldp;					/* 新空间地址*/
        if (rsize >= MIN_CHUNK_SIZE) {	/* 足够大则分割*/
      ...
      else if (next == m->top && oldsize + m->topsize > nb) {	/* top扩展*/
        /* Expand into top */
    ...
    if (newp != 0) {
      ...
      void* newmem = internal_malloc(m, bytes);		/* 重新分配*/
      if (newmem != 0) {
...
static void* internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {	/* 太大了*/
    ...
    if (mem != 0) {
      ...
      if ((((size_t)(mem)) % alignment) != 0) { /* misaligned */
        ...
        */
        ...
        char* br = (char*)mem2chunk((size_t)(((size_t)(mem +
                                                       ...
                                                       SIZE_T_ONE)) &
                                             -alignment));
		/* 第一个位置太小(小于MIN_CHUNK_SIZE)，则加上alignment
		，alignment是大于MIN_CHUNK_SIZE的。*/
      ...
      POSTACTION(m);
      if (leader != 0) {
        internal_free(m, leader);		/* 释放*/
      }
      if (trailer != 0) {
        internal_free(m, trailer);		/* 释放*/
      }
      return chunk2mem(p);
    }
  }
  ...
  */
  ...
  if (!PREACTION(gm)) {
    ...
    if (bytes <= MAX_SMALL_REQUEST) {
      ...
      if ((smallbits & 0x3U) != 0) { /* Remainderless fit to a smallbin. */
		...
		*/
        ...
        goto postaction;
          ...
          /* printf("gm->dvsize %d smallbits  %d\n", gm->dvsize, smallbits); */
		...
		/* 存在其它空闲的chunk块，则进行申请*/
		  ...
		  /* 获取smallbits中值为1的最小bit位掩码，也就是
		  最满足要求的空闲块。*/
          binmap_t leastbit = least_bit(leftbits);
		  /* i 取得leastbit的二进制中位为1的最小位置索引
		  从0开始
		  比如leastbit为0000 0100，则i为2。*/
          compute_bit2idx(leastbit, i);
		  /* 取得该最满足要求的空闲块。*/
          b = smallbin_at(gm, i);
          p = b->fd;
      ...
      /* printf("nb %d gm->treemap %d  gm->topsize : %d %p\n", nb, gm->treemap, gm->topsize, mem); */
    }
    else if (bytes >= MAX_REQUEST)
      nb = MAX_SIZE_T; /* Too big to allocate. Force failure (in sys alloc) */
    else {
	/* 请求的内存大小在
		MAX_SMALL_REQUEST和MAX_REQUEST之间。*/
    ...
    /* printf("gm->dvsize %d gm->topsize :%d line:%d \n", gm->dvsize, gm->topsize, __LINE__); */
    if (nb <= gm->dvsize) {
...
#define fm gm
    ...
    if (!PREACTION(fm)) {
      ...
      if (RTCHECK(ok_address(fm, p) && ok_cinuse(p))) {
		...
		不存在mmap的情况下，IS_MMAPPED_BIT为0
		那么下个if语句只有在存在mmap的情况下
		才有可能为真
		当前一块是通过mmap分配的时候，执行下个if语句
		*/
...
void* dlrealloc(void* oldmem, size_t bytes) {
  if (oldmem == 0)	/* 直接调用dlmalloc函数进行内存分配*/
    return dlmalloc(bytes);
#ifdef REALLOC_ZERO_BYTES_FREES
/* 当realloc(p,0)等同于调用free(p)的时候需要定义这个
宏；否则realloc(p,0)就与malloc(0)含义相同*/
  if (bytes == 0) {
    dlfree(oldmem);
    return 0;
  }
